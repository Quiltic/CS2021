...
def has_sublist(s, sublist):
    ...
    if sublist == empty:
        ...
        if first(s) != first(minilist):
            return(has_prefix(rest(s),minilist))
        else:
            while minilist != empty:
                if s == empty:
                    return(False)
                elif first(s) == first(minilist):
                    s = rest(s)
                    minilist = rest(minilist)
                else:
                    minilist = list(sublist)
                    s = rest(s)
            else:
...
def has_kitty_gene(dna):
    ...
    """
    "*** YOUR CODE HERE ***"
    test = link('C', link('A', link('T', link('C', link('A', link('T'))))))
    while dna != empty:
        if test == empty:
            return(True)
        elif first(dna) == first(test):
            test = rest(test)
...
def count_change(amount, denominations):
    ...
    total = count_change(amount,rest(denominations))

    coins = 1
    while first(denominations)*coins <= amount:
        total += count_change((amount-first(denominations)*coins),rest(denominations))
        coins += 1
